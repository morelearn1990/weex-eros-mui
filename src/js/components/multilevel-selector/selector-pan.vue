...
export default {
  ...
  methods: {
    ...
    bindTiming(index) {
      ...
      let listLength = list[index].data.length || 0;

      let changed_y, final_y, translate_y_origin;
      let yCurrent = y[index];
      const STEPHEIGHT = -100;
      if (yCurrent <= listLength * STEPHEIGHT) {
        final_y = listLength * STEPHEIGHT;
        changed_y = final_y - yCurrent;
      } else if (yCurrent > 0) {
        final_y = 0;
        changed_y = final_y - yCurrent;
      } else {
        let yRemainder = yCurrent % STEPHEIGHT;
        if (yRemainder > STEPHEIGHT / 2) {
          final_y = yCurrent - yRemainder;
          changed_y = -yRemainder;
        } else {
          final_y = yCurrent + STEPHEIGHT - yRemainder;
          changed_y = STEPHEIGHT - yRemainder;
        }
      }
      translate_y_origin = `easeOutQuad(t,${yCurrent},${changed_y},200)`;
    ...
    resetTiming(index) {
      ...
      this.isInAnimation[index] = true;
      let selectorRef = this.$refs[`selector${index}`][0].ref;
      let translate_y_origin = `easeOutQuad(t,${subY},${-subY},200)`;
      let result = this.$bindingx.bind(
        {
          eventType: "timing",
